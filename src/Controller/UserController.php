...
{
    private function getLoggedInUser(Request $request, EntityManagerInterface $entityManager): ?UserType
    {
        $email = $request->getSession()->get('user_email');
        if (!$email) {
            return null;
        }
        return $entityManager->getRepository(UserType::class)->findOneBy(['email' => $email]);
    }
    ...
    public function show(Request $request, UserType $user, EntityManagerInterface $entityManager): Response
    {
        $currentUser = $this->getLoggedInUser($request, $entityManager);
        if (!$currentUser || $currentUser->getRole() !== 'ROLE_ADMIN') {
            $this->addFlash('error', 'Accès non autorisé');
    ...
    public function edit(Request $request, UserType $user, EntityManagerInterface $entityManager): Response
    {
        $currentUser = $this->getLoggedInUser($request, $entityManager);
        if (!$currentUser || $currentUser->getRole() !== 'ROLE_ADMIN') {
            $this->addFlash('error', 'Accès non autorisé');
    ...
    public function delete(Request $request, UserType $user, EntityManagerInterface $entityManager): Response
    {
        $currentUser = $this->getLoggedInUser($request, $entityManager);
        if (!$currentUser || $currentUser->getRole() !== 'ROLE_ADMIN') {
            $this->addFlash('error', 'Accès non autorisé');
